document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderCart() {
        ...
        <td>
            <div class="quantity-controls">
                ...
                <span class="quantity-display" data-index="${index}">${cantidad}</span>
    ...
    `;
        ...
        // Evento para manejar los clics en los botones + y -
        cartItemsContainer.addEventListener("click", (event) => {
            if (event.target.classList.contains("quantity-btn")) {
                const index = event.target.dataset.index;
                const cartItems = getCartItems();
                const currentItem = cartItems[index];
                let newQuantity = currentItem.cantidad;

                // Si el botón es +, sumamos 1; si es -, restamos 1 (con un mínimo de 1)
                if (event.target.classList.contains("plus")) {
                    newQuantity++;
                } else if (event.target.classList.contains("minus") && newQuantity > 1) {
                    newQuantity--;
                }
                ...
                // Re-renderizar el carrito para mostrar los cambios
                renderCart();
            }
