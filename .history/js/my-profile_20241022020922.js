document.addEventListener("DOMContentLoaded", function () {
  ...
  if (savedImage) {
    profilePic.src = savedImage; // Muestra la imagen guardada
    userImage.src = savedImage; // Actualiza la imagen en el header
  } else {
    profilePic.src = defaultFile; // Imagen por defecto
    userImage.src = defaultFile; // Imagen por defecto
  ...
  fileInput.addEventListener('change', e => {
    if (e.target.files[0]) {
      ...
      reader.onload = function (e) {
        ...
        userImage.src = imageBase; // Actualiza la imagen en el header
