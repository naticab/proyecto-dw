document.addEventListener("DOMContentLoaded", function () {
  ...
  if (savedImage) {
    ...
    if (userImage) userImage.src = savedImage; // Actualiza la imagen en el header si existe
  } else {
    profilePic.src = defaultFile; // Imagen por defecto
    if (userImage) userImage.src = defaultFile; // Imagen por defecto si existe
  }
  ...
  fileInput.addEventListener('change', e => {
    if (e.target.files[0]) {
      ...
      reader.onload = function (e) {
        ...
        if (userImage) userImage.src = imageBase; // Actualiza la imagen en el header si existe
