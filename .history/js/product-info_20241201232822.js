...
document.addEventListener("DOMContentLoaded", async function () {
    // Recuperamos el ID del producto seleccionado desde localStorage
    ...
    updateCartCounter(); // Esta función actualizará el contador en el header
});

// Función para cargar los datos del producto
async function loadProductData(productId) {
    try {
        const response = await fetch(`https://japceibal.github.io/emercado-api/products/${productId}`);
...
function handleBuy() {
    ...
    const productoComprado = {
        ...
        precio: parseFloat(document.getElementById('price').textContent.split(' ')[1]), // Extraemos el precio
        moneda: document.getElementById('price').textContent.split(' ')[0], // Extraemos la moneda
    ...
    // Mostrar mensaje emergente de producto agregado
...
// Función para generar estrellas en los comentarios
