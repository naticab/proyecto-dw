document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    // Variables de las pestañas y campos
    const addressTab = document.getElementById("addressTab");
    const paymentTab = document.getElementById("paymentTab");
    const addressInputs = document.querySelectorAll("#address input[required]"); // Todos los campos obligatorios de la dirección
    const addressForm = document.querySelector("#address form");
  
    // Función para validar los campos de dirección
    function validateAddressFields() {
      ...
      addressInputs.forEach(input => {
        if (input.type === "radio" && !input.checked) {
          allValid = false; // Si algún campo radio no está seleccionado, marca como inválido
          input.classList.add("is-invalid"); // Agregar clase de error
        } else if (input.value.trim() === "") {
          allValid = false; // Si el campo está vacío, marca como inválido
          input.classList.add("is-invalid");
        } else {
          input.classList.remove("is-invalid"); // Si el campo es válido, quitar clase de error
        }
      });
    ...
    // Función para habilitar la pestaña de pago si todos los campos de dirección son válidos
    function togglePaymentTab() {
      if (validateAddressFields()) {
        paymentTab.classList.remove("disabled"); // Habilitar la pestaña de pago
      } else {
        paymentTab.classList.add("disabled"); // Deshabilitar la pestaña de pago
      }
    }
  
    // Añadir eventos a los campos de dirección para validar en tiempo real
    addressInputs.forEach(input => {
      input.addEventListener("input", function () {
        togglePaymentTab(); // Validar y habilitar la pestaña de pago cada vez que se modifique un campo
      });
    });
  
    // Inicializar la validación al cargar la página
    togglePaymentTab(); // Ejecutar la validación al cargar para asegurarse de que la pestaña de pago esté deshabilitada al inicio
  });
  ...
  document.getElementById('finalize-purchase').addEventListener('click', function (event) {
    ...
    updateCartCount(); // Esta función está en contador.js y actualiza el contador
